...
class Bank {
    ...
    int totalBalance(int acc[]) throws InvalidAccount {
        int total = 0;
        for (int i = 0; i < N; i++) {
    ...
    void transfer(int from, int to, int amount) throws InvalidAccount, NotEnoughFunds {
        if (from < 0 || from >= N) throw new InvalidAccount();
        if (to < 0 || to >= N) throw new InvalidAccount();
...
class Main {
    public static void main(String args[]) throws InterruptedException, InvalidAccount, NotEnoughFunds {
        ...
        for (int i = 0; i < N; i++) System.out.println(i + " - " + b.accountBalance(i));
